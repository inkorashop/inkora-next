...
- **"Ver pedido" (admin, `app/admin/page.js`)**: última fila de la tabla de items ("+ Agregar diseño"). Llama a `add_order_extra_design(..., p_added_via: 'pedido')`.
- **Producción (`components/ProductionTab.js`)**: última fila de la tabla de tareas del pedido seleccionado, visible para admin **y** operario. Llama a la misma RPC con `p_added_via: 'produccion'`.

(Originalmente el disparador era un botón aparte arriba de la tabla; se movió a ser la última fila de la tabla misma a pedido del usuario, sin cambiar la lógica de datos.)

Ambos entry points comparten el mismo mini-formulario (`components/AddExtraDesignForm.js`, fuzzy search sobre `designs` + stepper de cantidad) y el mismo ícono de auditoría hover (`components/InfoTooltip.js`, reescrito luego para usar un portal a `document.body` y no quedar recortado por el scroll de la tabla).
...
La columna **"a producir" de cada fila** (en la tabla de tareas de Producción) nunca muestra ningún desglose — siempre es el número plano `required_qty` de esa fila, misma lógica de siempre. **La tarjeta resumen "A producir" de arriba tampoco** — se probó mostrar ahí un desglose tipo "100 + 2" cuando había agregados vía Producción, pero se revirtió a pedido explícito del usuario (cambio de idea, no un bug): siempre muestra el total plano, sin diferenciar. Los campos `added_qty`/`added_via`/etc. se siguen guardando igual (los sigue usando el ícono "i" y la edición inline), solo que ya no se usan para partir el número del resumen.
...
## Edición inline de una fila agregada (diseño + cantidad)

`edit_order_extra_design(p_order_id, p_old_design_id, p_new_design_id, p_new_qty)` (`sql/edit_order_extra_design.sql`) deja corregir, sin borrar y recargar, una fila ya agregada — tanto el diseño como la cantidad. Se dispara sola (sin botón "guardar"): clic en el nombre del diseño abre un buscador inline; salir del input de cantidad la guarda al perder foco. Implementado igual en Producción y en "Ver pedido".

Puntos importantes:

- **Identifica la fila por `(order_id, design_key viejo)`, no por el id de la tarea**: el modal "Ver pedido" solo conoce `orders.items` (que no tiene el id de `production_order_tasks`), así que ambos llamadores pueden usar la misma función con lo que cada uno ya tiene a mano (Producción tiene `task.design_id`, Pedido tiene `item.design_id`).
- **Solo edita filas "puramente agregadas"** (`added_qty = required_qty`, o sea toda la cantidad de la fila vino de una adición, no una mezcla con cantidad original del pedido). Si se intenta sobre una fila mixta, rechaza con un mensaje explícito — no hay una forma no ambigua de "reasignar" solo la parte agregada de una fila mezclada. La UI no expone la edición para esas filas (no debería aparecer nunca ese rechazo en la práctica).
- **Si el diseño nuevo elegido ya tiene su propia fila en el pedido**, la función fusiona ahí (suma las cantidades) y borra la fila vieja, para no violar el `UNIQUE(order_id, design_key)` de `production_order_tasks`.
...
- `sql/edit_order_extra_design.sql` — la función de edición inline. Ya aplicada.
- `components/AddExtraDesignForm.js` — mini-formulario compartido (fuzzy search + qty) para agregar.
- `components/InfoTooltip.js` — ícono "i" hover genérico (fecha/hora + usuario), sin click, se cierra al instante al sacar el mouse. Portal a `document.body`, no queda recortado por el scroll de una tabla.
- `app/admin/page.js` — fila de agregar + resaltado + edición inline en el modal "Ver pedido" (buscar `addExtraDesignToOrder`, `editAddedOrderDesign`).
- `components/ProductionTab.js` — fila de agregar + resaltado + edición inline en Producción (buscar `addExtraDesignToOrder`, `editAddedDesign`).
