 ...
 * returns top N matches sorted by score descending. `productOrderById`
 * (Map de product_id -> posicion en el catalogo) desempata resultados con
 * el mismo score segun el orden real de productos, en vez del orden de
 * llegada de `designs` (que no tiene por que coincidir con el catalogo).
 */
export function fuzzyMatchDesigns(query, designs, topN = 5, productOrderById = null) {
  if (!query || !designs?.length) return [];
  const orderOf = d => {
    if (!productOrderById || d.product_id == null) return Number.MAX_SAFE_INTEGER;
    const idx = productOrderById.get(d.product_id);
  ...
  return designs
    ...
    .sort((a, b) => b.score - a.score
      || orderOf(a.design) - orderOf(b.design)
      // Ultimo desempate: orden "natural" (numerico), como ordenaria un
      // explorador de archivos — "Argentina 2" antes que "Argentina 10" —
      // en vez del orden de llegada de `designs`.
      || a.design.name.localeCompare(b.design.name, undefined, { numeric: true, sensitivity: 'base' }))
    .slice(0, topN);
}
