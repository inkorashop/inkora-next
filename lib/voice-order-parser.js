...
// Handles compound number words ("treinta y dos" = 30+2, "ciento veinte" = 100+20)
// by summing each recognized component word. Bails to null if any word is unknown,
// rather than guessing — the caller falls back to a default qty of 1.
function parseQtyWord(text) {
  const t = text.trim();
  if (!t) return null;
  const n = parseInt(t, 10);
  if (!isNaN(n) && n > 0) return n;
  const words = t.split(/\s+/).map(normWord).filter(w => w && w !== 'y');
...
export function parseSpanishDate(text) {
  ...
  let m = t.match(/(\d{1,2})\s+de\s+([a-z]+)(?:\s+del?\s+(\d{2,4})|\s+(\d{2,4}))?/);
  if (m) {
