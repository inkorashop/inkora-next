...
export async function removeBridgePdfRoot(baseUrl, token, path) {
  return bridgeFetch('/pdf-roots/remove', {
    baseUrl,
    token,
    method: 'POST',
    timeoutMs: 10000,
...
// A diferencia de bridgeFetch (siempre espera JSON), esta pide los bytes
// reales del PDF para mostrarlo en el visor flotante sin subirlo a la web.
export async function fetchBridgePdfBlob(baseUrl, token, { rootName, relativePath }, timeoutMs = 15000) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);

  try {
    const query = new URLSearchParams({ rootName: rootName || '', relativePath: relativePath || '' });
    const response = await fetch(`${normalizeBridgeUrl(baseUrl)}/pdf-file?${query.toString()}`, {
      headers: { 'X-Inkora-Bridge-Token': token || '' },
      signal: controller.signal,
      cache: 'no-store',
    });
    ...
    if (!response.ok) {
      let message = `Bridge HTTP ${response.status}`;
      try {
        const payload = await response.json();
        message = payload?.error || message;
      } catch {
        // La respuesta de error no era JSON: se usa el mensaje generico.
      }
      const error = new Error(message);
      error.status = response.status;
      throw error;
...
export async function selectBridgePrintFile(baseUrl, token) {
  return bridgeFetch('/print-file/select-dialog', {
    baseUrl,
    token,
    method: 'POST',
    timeoutMs: 120000,
...
export async function printBridgeFile(baseUrl, token, { fullPath, printerName, copies }) {
  return bridgeFetch('/print-file', {
    baseUrl,
    token,
    method: 'POST',
    timeoutMs: 60000,
    body: { fullPath, printerName, copies },
  });
}
...
export async function getBridgeUpdateStatus(baseUrl, token) {
  return bridgeFetch('/update/status', { baseUrl, token, timeoutMs: 3000 });
}
