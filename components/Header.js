  ...
  return (
    <header style={{ background: '#1B2F5E', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 16px rgba(27,47,94,0.25)', transform: headerVisible ? 'translateY(0)' : 'translateY(-100%)', transition: 'transform 0.3s ease' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
        <a href="/" className="header-btn" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', borderRadius: 12, transition: 'box-shadow 0.3s ease, transform 0.3s ease' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.08)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="789 194 2978 5222" style={{ height: 36, width: 'auto', fill: 'white' }}>
