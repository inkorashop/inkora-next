...
export default function Home() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setCartBottom(Math.max(8, overlap + 8));
    };
  ...
  function beginCategoryEdit(cat, anchor, event) {
    if (!isAdmin || !activeProductId || cat === 'Todos' || cat === 'Sin categoria' || cat === 'Sin categoría') return;
    event?.preventDefault();
    event?.stopPropagation();
    setEditingCategory({ productId: activeProductId, oldName: cat, value: cat, anchor });
  }

  function updateLocalCategoryName(productId, oldName, newName, nextCategories, nextColors) {
    setProducts(prev => prev.map(p => p.id === productId ? {
      ...p,
      categories: nextCategories,
      category_colors: nextColors,
    } : p));
    setDesignsByProduct(prev => {
      const productDesigns = prev[productId] || [];
      return {
        ...prev,
        [productId]: productDesigns.map(d => {
          const nextDesignCategories = Array.isArray(d.categories)
            ? [...new Set(d.categories.map(c => c === oldName ? newName : c))]
            : d.categories;
          return {
            ...d,
            category: d.category === oldName ? newName : d.category,
  ...
  async function saveCategoryRename() {
    ...
    const currentCategories = Array.isArray(product?.categories) && product.categories.length > 0
      ? product.categories
      : [...new Set((designsByProduct[edit.productId] || []).map(d => d.category).filter(c => c && c !== 'Sin categoria' && c !== 'Sin categoría'))];
    const nextCategories = [...new Set(currentCategories.map(c => c === edit.oldName ? newName : c))];
    const currentColors = product?.category_colors || {};
    const nextColors = { ...currentColors };

    if (Object.prototype.hasOwnProperty.call(nextColors, edit.oldName)) {
      nextColors[newName] = nextColors[newName] || nextColors[edit.oldName];
      delete nextColors[edit.oldName];
    }

    const previousProducts = products;
    const previousDesignsByProduct = designsByProduct;
    updateLocalCategoryName(edit.productId, edit.oldName, newName, nextCategories, nextColors);
    if (filter === edit.oldName) setFilter(newName);
    setEditingCategory(null);
    ...
    try {
      ...
      const { data: productDesigns, error: designsError } = await supabase
        .from('designs')
        .select('id, category, categories')
        .eq('product_id', edit.productId)
        .limit(10000);
      if (designsError) throw designsError;

      const updates = (productDesigns || [])
        .filter(d => d.category === edit.oldName || (Array.isArray(d.categories) && d.categories.includes(edit.oldName)))
        .map(d => {
          const nextDesignCategories = Array.isArray(d.categories)
            ? [...new Set(d.categories.map(c => c === edit.oldName ? newName : c))]
            : d.categories;
          const payload = {
            category: d.category === edit.oldName ? newName : d.category,
          };
          if (Array.isArray(nextDesignCategories)) payload.categories = nextDesignCategories;
          return supabase
            .from('designs')
            .update(payload)
            .eq('id', d.id);
      ...
      setDesignsByProduct(previousDesignsByProduct);
      if (filter === newName) setFilter(edit.oldName);
        ...
        <div style={s.catalogArea}>
          ...
          <div style={{...s.filters, minHeight: isMobile ? 36 : 40, alignItems:'center'}}>
            {categories.length === 0 ? (
              ...
              const editAnchor = `filter:${cat}`;
              const isEditing = editingCategory?.productId === activeProductId && editingCategory?.anchor === editAnchor;
              if (isEditing) {
                return (
                  <input
                    key={cat}
                    ...
                    style={{
                      ...
                      boxShadow: '0 2px 8px rgba(27,47,94,0.22)',
                    }}
                    value={editingCategory.value}
                    disabled={savingCategory}
                    onClick={e => e.stopPropagation()}
                    onChange={e => setEditingCategory(prev => prev ? { ...prev, value: e.target.value } : prev)}
                    onBlur={saveCategoryRename}
                    onKeyDown={e => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
              ...
              return (
                <button
                  ...
                  onDoubleClick={e => beginCategoryEdit(cat, editAnchor, e)}
                  title={isAdmin && cat !== 'Todos' ? 'Doble click para renombrar' : undefined}
                >
                  {cat}
                </button>
              );
                ...
                return (
                  ...
                  >
                    ...
                    <div style={s.cardBody}>
                      ...
                      {d.category !== 'Sin categoria' && (() => {
                        const editAnchor = `card:${d.id}`;
                        const isEditing = editingCategory?.productId === activeProductId && editingCategory?.anchor === editAnchor;
                        if (isEditing) {
                          return (
                            <input
                              autoFocus
                              style={{...s.catTag, ...s.categoryEditInput, width: Math.max(80, editingCategory.value.length * 8)}}
                              value={editingCategory.value}
                              disabled={savingCategory}
                              onClick={e => e.stopPropagation()}
                              onChange={e => setEditingCategory(prev => prev ? { ...prev, value: e.target.value } : prev)}
                        ...
                        return (
                          <span
                            style={{...s.catTag, cursor: isAdmin ? 'text' : 'default'}}
                            onDoubleClick={e => beginCategoryEdit(d.category, editAnchor, e)}
            ...
            {!success ? (
              ...
              <div style={s.successScreen}>
                ...
                >
                  ...
                  {codeCopied && (
                    <span className="copied-overlay" style={{position: 'absolute', inset: 0, background: 'rgba(24,163,106,0.6)', backdropFilter: 'blur(2px)', WebkitBackdropFilter: 'blur(2px)', color: 'white', textShadow: '0 1px 3px rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 14, fontWeight: 800, borderRadius: 8, pointerEvents: 'none'}}>
                ...
                {profile?.send_confirmation_email !== false && (
                  <p style={{color:'#9aa3bc', fontSize:13}}>Te enviamos la confirmacion a tu email.</p>
                )}
                <button
                  style={{marginTop:16, width:'100%', maxWidth:300, background:'#1B2F5E', border:'none', color:'white', borderRadius:10, padding:'12px 24px', fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:'Barlow, sans-serif', boxShadow:'0 8px 20px rgba(27,47,94,0.16)', whiteSpace:'nowrap'}}
                  onClick={closeModal}
                >
                  Listo ✓
                </button>
                    ...
                    <div style={{marginTop:22, borderTop:'1.5px solid #eef0f6', paddingTop:18, textAlign:'left'}}>
                      <div style={{background:'#f8fdf9', border:'1.5px solid #c7f4d8', borderRadius:12, padding:16}}>
                        <a
                          ...
                          rel="noreferrer"
                          onClick={closeModal}
                          style={{...s.btnWaConfirm, display:'flex', width:'100%', maxWidth:300, margin:'0 auto', justifyContent:'center', padding:'13px 20px', borderRadius:11, background:'#25D366', color:'white', border:'none', fontSize:15, fontWeight:800, boxShadow:'0 8px 20px rgba(37,211,102,0.30)', whiteSpace:'nowrap'}}
                        >
                        ...
                        >
                          ...
                          {waCopied && (
                            <span className="copied-overlay" style={{position:'absolute', inset:0, background:'rgba(24,163,106,0.6)', backdropFilter:'blur(2px)', WebkitBackdropFilter:'blur(2px)', color:'white', textShadow:'0 1px 3px rgba(0,0,0,0.35)', display:'flex', alignItems:'center', justifyContent:'center', gap:8, fontSize:14, fontWeight:800, borderRadius:10, pointerEvents:'none'}}>
...
const styles = {
  ...
  categoryEditInput: { border: '1.5px solid rgba(45,107,228,0.55)', outline: 'none', fontFamily: 'Barlow, sans-serif', minWidth: 86, maxWidth: 180, cursor: 'text', boxSizing: 'border-box', textTransform: 'none', letterSpacing: 0 },
  ...
  sidebarHeader: { background: '#1B2F5E', color: 'white', padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  sidebarTitle: { fontWeight: 700, fontSize: 15, letterSpacing: 1 },
  badge: { background: '#2D6BE4', color: 'white', fontSize: 11, fontWeight: 700, padding: '2px 7px', borderRadius: 10 },
  sidebarBody: { padding: '10px 14px', flex: 1, overflowY: 'auto', minHeight: 0 },
  cartEmpty: { textAlign: 'center', padding: '16px 12px', color: '#9aa3bc', fontSize: 13 },
  cartItem: { display: 'flex', alignItems: 'center', gap: 7, padding: 7, background: '#f7f8fc', borderRadius: 8, marginBottom: 5 },
  ...
  cartQty: { fontSize: 11, fontWeight: 700, color: '#1B2F5E', background: '#e8eef9', borderRadius: 6, padding: '2px 7px' },
  cartPrice: { fontSize: 11, color: '#5a6380' },
  removeBtn: { background: 'none', border: 'none', cursor: 'pointer', color: '#9aa3bc', fontSize: 13 },
  sidebarFooter: { padding: '10px 14px', borderTop: '1.5px solid #eef0f6' },
