...
useEffect(() => {
  ...
  const [orderContactForm, setOrderContactForm] = useState({ name: '', phone: '' });
  const [savingOrderContact, setSavingOrderContact] = useState(false);
  const [savedOrderContact, setSavedOrderContact] = useState(false);
  const [orderContactMsg, setOrderContactMsg] = useState('');
  ...
  useEffect(() => {
    if (!orderDetail) {
      setOrderContactForm({ name: '', phone: '' });
      setSavingOrderContact(false);
      setSavedOrderContact(false);
      setOrderContactMsg('');
      return;
    }
  ...
  async function saveOrderCustomerContact() {
    if (!orderDetail) return;

    const email = (orderDetail.customer_email || '').trim().toLowerCase();
    const name = (orderContactForm.name || '').trim();
    const phone = (orderContactForm.phone || '').replace(/[^0-9]/g, '');

    if (!email) {
      setOrderContactMsg('Este pedido no tiene email asociado.');
      return;
    }

    if (!name) {
      setOrderContactMsg('El nombre de comercio no puede quedar vacío.');
      return;
    }

    setSavingOrderContact(true);
    setSavedOrderContact(false);
    setOrderContactMsg('');

    const { data: profileUpdated, error: profileError } = await supabase.rpc('admin_update_customer_contact', {
      p_email: email,
      p_name: name,
      p_phone: phone,
    });
    ...
    if (profileError) {
      setSavingOrderContact(false);
      setOrderContactMsg(`No se pudo actualizar el usuario: ${profileError.message}`);
      return;
    }
    ...
    if (orderError) {
      setSavingOrderContact(false);
      setOrderContactMsg(`Se actualizó el usuario, pero no el pedido actual: ${orderError.message}`);
      return;
    }
    ...
    setOrders(prev => prev.map(order => (
      order.id === orderDetail.id
        ? { ...order, customer_name: name, customer_phone: phone || null }
        : order
    )));

    setUsers(prev => prev.map(user => (
      String(user.email || '').toLowerCase() === email
        ? { ...user, name, phone: phone || null }
        : user
    )));

    trackAdminActivity('order_customer_contact_update', {
      order_id: orderDetail.id,
      order_code: orderDetail.order_code,
      customer_email: email,
      profile_updated: Boolean(profileUpdated),
    }, 'orders');

    setSavingOrderContact(false);
    setSavedOrderContact(true);
    setOrderContactMsg(profileUpdated
      ? 'Datos guardados en el usuario y en el pedido actual.'
      : 'Pedido actualizado. No se encontró un perfil de usuario con ese email.'
    );

    setTimeout(() => setSavedOrderContact(false), 1800);
  }
        ...
        <div style={{position:'fixed', inset:0, background:'rgba(17,32,64,0.55)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:20}} onClick={() => setOrderDetail(null)}>
          <div style={{background:'white', borderRadius:16, border:'1.5px solid #dde1ef', boxShadow:'0 8px 40px rgba(27,47,94,0.18)', padding:'28px 28px 24px', width:'100%', maxWidth:520, maxHeight:'80vh', overflowY:'auto', display:'flex', flexDirection:'column', gap:14}} onClick={e => e.stopPropagation()}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px 16px', fontSize:13}}>
              <div>
                <span style={{color:'#9aa3bc', fontSize:11, fontWeight:600, textTransform:'uppercase'}}>Nombre de comercio</span>
                <input
                  style={{
                    width:'100%',
                    border:'1.5px solid #dde1ef',
                  ...
                  value={orderContactForm.name}
                  disabled={savingOrderContact}
                  onChange={e => setOrderContactForm(prev => ({ ...prev, name: e.target.value }))}
                  onKeyDown={e => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      saveOrderCustomerContact();
                    }
                  }}
                />
              </div>
              ...
              <div>
                <span style={{color:'#9aa3bc', fontSize:11, fontWeight:600, textTransform:'uppercase'}}>Teléfono</span>
                <input
                  style={{
                    width:'100%',
                    border:'1.5px solid #dde1ef',
                  ...
                  type="tel"
                  inputMode="numeric"
                  value={orderContactForm.phone}
                  disabled={savingOrderContact}
                  onChange={e => setOrderContactForm(prev => ({
                    ...prev,
              ...
              <div>
                <span style={{color:'#9aa3bc', fontSize:11, fontWeight:600, textTransform:'uppercase'}}>Email</span>
                <div style={{fontWeight:600, color:'#2d3352'}}>{orderDetail.customer_email || '—'}</div>
              </div>

              <div>
                <span style={{color:'#9aa3bc', fontSize:11, fontWeight:600, textTransform:'uppercase'}}>Fecha</span>
                <div style={{fontWeight:600, color:'#2d3352'}}>{orderDetail.created_at ? new Date(orderDetail.created_at).toLocaleString('es-AR') : '—'}</div>
              </div>

              <div style={{gridColumn:'1/-1', display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
                <button
                  type="button"
                  onClick={saveOrderCustomerContact}
                  disabled={savingOrderContact || !orderContactForm.name.trim() || !orderDetail.customer_email}
                  style={{
                    border:'none',
                    borderRadius:8,
                    padding:'7px 12px',
                    ...
                    fontSize:12,
                    fontWeight:700,
                    cursor: savingOrderContact || !orderContactForm.name.trim() || !orderDetail.customer_email ? 'not-allowed' : 'pointer',
                    opacity: savingOrderContact || !orderContactForm.name.trim() || !orderDetail.customer_email ? 0.55 : 1,
                  }}
                >
                  {savingOrderContact ? 'Guardando...' : 'Guardar datos del cliente'}
                </button>

                {savedOrderContact && (
                  <span style={{fontSize:12, fontWeight:700, color:'#15803d'}}>✓ Guardado</span>
                )}
                ...
                {orderContactMsg && (
                  <span style={{
                    fontSize:12,
                    fontWeight:600,
                    color: orderContactMsg.includes('No se pudo') || orderContactMsg.includes('no puede') || orderContactMsg.includes('no tiene')
                      ? '#b91c1c'
                      : '#5a6380',
                  }}>
                    {orderContactMsg}
              ...
              {orderDetail.notes && (
                <div style={{gridColumn:'1/-1'}}>
                  <span style={{color:'#9aa3bc', fontSize:11, fontWeight:600, textTransform:'uppercase'}}>Notas</span>
                  <div style={{fontWeight:500, color:'#5a6380'}}>{orderDetail.notes}</div>
                </div>
              )}
            </div>
            <div>
