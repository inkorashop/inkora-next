...
- `PDF_MANUAL_LINK_FEATURE.md`: como funciona el vinculo manual diseno-PDF (boton "🎯" en Disenos), para cuando el emparejamiento automatico por nombre contra los PDFs escaneados por el Bridge falla o elige mal. Leer antes de tocar `refreshDesignPdfLinks`/`manualPdfMatchFor` en `app/admin/page.js`, o antes de asumir que el match diseno-PDF que ve Produccion siempre es automatico.
